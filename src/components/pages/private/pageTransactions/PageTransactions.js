...
const PageTransactions = observer(() => {
    ...
    //         .then((data) => {
    //             console.log("transact", data.transactions)
    //             // profileStore.setProfile(data);
    //             transactStor.addTransaction(data.transactions);
    //             // data.transactions.forEach((items) => transactStor.addTransaction(items))
    //         })
        ...
        <div className='container'>
            ...
            <div className='transactions-buttons' >
                <div className='transactions-btn'>
                    {/* <button onClick={() => transactStor.setViewIn(false)} text={"Expenses"} ></button>
                    <button onClick={() => transactStor.setViewIn(true)} text={"Incomes"}></button> */}
                     {/* <button onClick={() => transactStor.setViewIn(false)}> Expenses</button>  */}
                     <div className='send-request-transactions'>
                {/* <div className='row'>
                    <div className='col' style={{ padding: " 0 auto" }}>
                        <PrimaryBtnYellow onClick={() => transactStor.setViewIn(true)} text={"Send Money"}  />
                    </div>
                    <div className='col'>
                        <PrimaryBtn onClick={() => transactStor.setViewIn(false)} text={"Request"}  />
                    </div>
                </div> */}
                  <div className='row'>
        <div className='col' style={{ padding: " 0 auto" }}>
